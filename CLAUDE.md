...
> **적용 경계 (2026-08-09 확정).** 이 절의 분담은 **설계결정기록 009 이후의 결정부터 실제로 적용된다.**
> 그 이전에 작성된 `docs/GDD.md`·`docs/INVENTORY_DESIGN.md`·설계결정기록 000·설계결정기록 002는 AI가 초안과
> 근거를 제시하고 사용자가 읽고 납득한 산출물이며, 사용자가 스스로 선택지를 훑고 내린 판단이
> 아니다. 설명 역전으로 측정한 결과 설계결정기록 002에서 사용자가 재생할 수 있었던 것은 결론 명사
...
> 식별자 결정만은 예외적으로 재결정했고 그 결과가 설계결정기록 009다.

### 5.1 설계 결정은 `질문 → 내 판단 → AI 리뷰 → 확정` 순서

설계결정기록이나 공개 API처럼 구조를 바꾸는 결정에서는 **AI가 결론을 먼저 제시하지 않는다.**
...
6. **AI — 설계결정기록 작성자**
   - 확정된 판단만 설계결정기록으로 정리한다.
   - 사용자가 말하지 않은 이유를 사후에 만들어 넣지 않는다.
7. **AI — Implementer**
   - 승인된 설계결정기록과 공개 계약 안에서만 구현한다.

사용자가 명시적으로 "추천해줘"라고 요청한 경우를 제외하면, 설계 단계의 기본 동작은
**결론 제시가 아니라 질문과 반론 제공**이다.

### 5.2 설계결정기록은 구현을 구속한다

승인된 설계결정기록은 참고 문서가 아니라 **Architecture Constraint**다.

- 구현 중 설계결정기록과 충돌하는 변경이 필요해 보여도 **임의로 바꾸지 않는다.**
- 먼저 아래를 보고하고 구현을 멈춘다.
  1. 충돌하는 설계결정기록 또는 공개 계약
  2. 필요한 변경
  3. 변경이 필요한 이유
  4. 설계결정기록을 유지하는 대안과 그 비용
- 사용자가 승인하면 설계결정기록을 먼저 수정하거나 `Superseded` 처리한 뒤 구현한다.
- "구현하기 편해서"는 설계결정기록 변경의 근거가 아니다.
...
- 모듈 신설은 구조 결정이므로 M1 시작 시 설계결정기록으로 남긴다.
...
- **AI가 설계안·근거·추천·설계결정기록까지 한 번에 완성하고 사용자가 승인만 하는 흐름** — 결정권이 AI로 이동한다.
...
| `docs/architecture/*.md` | 구조 결정 기록 — 단 `TEMPLATE.md`는 결정 기록이 아닌 빈 템플릿이고 `MODULE_MAP.md`는 참고 문서이므로 이 목록에서 제외한다 |
...
- **새 시스템·모듈 경계·데이터 흐름·패턴 선택이 걸린 결정은 설계결정기록으로 남긴다.**
  세션은 휘발되지만 설계결정기록은 누적된다.
- `INVENTORY_DESIGN.md`의 A1~E3와 설계결정기록 001~008은 **전부 `Proposed`다.** 임의로 확정하지 않는다.
- 설계결정기록은 최소한 아래 사고 흔적을 남긴다. 문장 형식은 자유지만 항목을 생략해 결론만 남기지 않는다.
...
- 이 항목들은 `docs/architecture/TEMPLATE.md`에 빈 템플릿으로 있다. **템플릿을 복사해
  `User Initial Decision`과 `User Reasoning / Concerns`를 사용자가 먼저 채운 뒤에만 AI가 이어 쓴다.**
  산문으로 적힌 규칙보다 빈칸이 있는 파일이 더 잘 지켜진다.
- 기존 설계결정기록 000·설계결정기록 002는 이 포맷이 정해지기 전에 작성됐다. **소급 적용하지 않는다** —
  해당 설계결정기록을 개정하거나 `Superseded` 처리할 때 템플릿으로 옮긴다.
...
- **`내가 결정한 것`은 요약하지 않고 해당 설계결정기록의 `User Reasoning / Concerns` 원문을 인용한다.**
...
- 작업 기록은 사실의 누적이다. 설계결정기록과 역할이 다르다 —
  **설계결정기록은 "왜 이 구조인가"(결정), 작업 기록은 "무엇을 어떻게 했는가"(경위).**

## 8. 마일스톤과 스코프

| 단계 | 내용 | 상태 |
| --- | --- | --- |
| M0 | 설계 (설계결정기록 승인) | 문서 완료, 설계결정기록 미승인 |
...
### 설계결정기록 승인 — 필요한 것만, 필요한 시점에

설계 문서는 설계결정기록 001~008을 **전부** M1 전에 승인하라고 적었지만, 실제로 M1(Model)에
필요한 것은 그중 일부다. **정보 없이 결정하면 나중에 `Superseded` 처리하게 된다** —
특히 설계결정기록 006(드래그 구현 경로)은 M2에서 실제로 만들어봐야 판단할 수 있다.
...
- 설계결정기록 000(모듈 경계)도 아직 `Proposed`다.
- 설계결정기록 승인에는 **5.1의 설계 결정 절차를 반드시 적용한다.** AI는 질문·선택지·검토 축을 먼저 제시하고,
